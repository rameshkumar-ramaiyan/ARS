...
/****** Object:  StoredProcedure [dbo].[uspgetAllNPDocumentsStrategicVersionOnNPNumber]    Script Date: 4/11/2016 7:07:25 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[uspgetAllNPDocumentsStrategicVersionOnNPNumber]
...
select  @DocPage= cast( DocPage as nvarchar(max)) from  sitepublisherii.dbo.DocPages where DocVer_ID =(select currentversion_id from sitepublisherii.dbo.Documents
 where Title like '%Strategic Vision%' and rtrim(ltrim(OriginSite_ID))=rtrim(ltrim(@NPNumber) )and published =  'p'and SPSysEndTime is  null)
--179
...
--4.select cast( DocPage as nvarchar(max)) from  sitepublisherii.dbo.DocPages where DocVer_ID =(179)and CurrentVersion = 1
GO

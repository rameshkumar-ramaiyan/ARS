...
namespace USDA_ARS.Core
{
    public class PullDataFromAccess
    {
        ...
        public string SetValues(string connectionStr, string tableName,bool storeInDB = true, bool bodyHtmlOnly = false)
        {
            ...
            bool matchingTable= TableExists(accessConnectionString, accessTableName);
            if (matchingTable == false)
            {
                 errorMessage = "ERROR---The given table does not exist in the Access database.";
                return errorMessage;
            }
            else
            {
        ...
        public static bool TableExists( string accessConnectionString,string accessTableName)
        {
            OleDbConnection conn = new OleDbConnection();
            conn.ConnectionString = accessConnectionString;
            conn.Open();
            var exists = conn.GetSchema("Tables", new string[4] { null, null, accessTableName, "TABLE" }).Rows.Count > 0;
            conn.Close();
            return exists;
        }

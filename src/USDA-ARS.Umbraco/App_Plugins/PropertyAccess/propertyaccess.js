		 	...
		 	if (event.animationName == 'nodeInserted') {
		 		...
		 		if (scope && $(event.target).hasClass('umb-property')) {
		 			...
		 			if (scope.property && scope.property.config) {
		 				...
		 				// Hide Properties
		 				if (scope.property.config.hideproperties) {
		 					$(event.target).hide();
		 				}

		 				// Disable Properties
		 				if (scope.property.config.disableproperties) {
		 					$(event.target).addClass('bck-tweak-disable');
		 				}

						// Force Preview Button on Nav Items
		 				if (scope.property.alias && scope.property.alias == "navigationItems") {
		 					...
		 					for (var i = 0; i < buttons.length; i++) {
		 						switch (buttons[i].toLowerCase()) {
		 							case 'preview':
		 								...
		 								break;
		 							case 'linktofile':
		 								// Hide link to file buttons
		 								var linktofileButtons = $('a').filter(function (index) { return $(this).text() === "Link to file"; })
		 								linktofileButtons.each(function () {
		 									$(this).addClass("hidden-button")
		 								});
		 								break;
		 							case 'actions':

...
var selectedColumnsField = '';
var appPluginsUrl = '';

$(document).ready(function () {

    $(".addQueryParameterOption").click(function () {
        UmbClientMgr.openModalWindow(appPluginsUrl+'/bulkmanager/pages/selectfieldsdialog.aspx?id=' + currentSelectedProvider + '&fields=' + getSelectedProviderFields(), 'Add filter criteria ', true, 500, 400);
        return false;
    });

    $(".outputColumnsSelector").click(function () {
        UmbClientMgr.openModalWindow(appPluginsUrl + '/bulkmanager/pages/selectcolumnsdialog.aspx?id=' + currentSelectedProvider + '&columns=' + getSelectedColumnFields(), 'Select output columns ', true, 500, 400);
        return false;
    });
...
function openBulkActionDialog(bulkActionProvider) {
    ...
    if (bulkActionProvider != '' && getSelectedRecordIds() != '') {
        UmbClientMgr.openModalWindow(appPluginsUrl + '/BulkManager/pages/bulkactiondialog.aspx?actionId=' + bulkActionProvider, $(".bulkActionDropdown :selected").text(), true, 500, 400);
    }
    return false;
}
...
function setSelectedColumnsField(field) {
    selectedColumnsField = field;
}

function setPluginFolder(pluginFolder) {
    appPluginsUrl = pluginFolder;
}
...
function getSelectedColumnFields() {
    return $('#' + selectedColumnsField).val();
}

function setSelectedColumnFields(selectedFields) {
    $('#' + selectedColumnsField).val(selectedFields);
}
...
function showError() {
    $('#dialogExecutingMessage').text('Cannot make a Ajax Call to /app_plugins/bulkmanager/pages/bulkactiondialog.aspx make sure request is not redirected');
    $('.progressBar').hide();
    $('.bulkActionFinished').show();
}
...
$(document).ready(function () {
    $(".draggable").draggable({
        revert: function (dropped) {
            var dropped = dropped && dropped[0].hasClass("draggable");
            return !dropped;
        },
        revertDuration: 0,
    ...
    $(".draggable").droppable({
        drop: function (event, ui) {
            ...
            if (droppedOn.hasClass("draggable")) {

                var table = droppedOn.parents('table.bulkmanagerOverviewTable');

                var sourceAlias = dropped.attr("fieldAlias");
                var targetAlias = droppedOn.attr("fieldAlias");
                dropped.attr("fieldAlias", targetAlias);
                droppedOn.attr("fieldAlias", sourceAlias);
                
                table.find("th.draggable").each(function (i) {
                    selected.push($(this).attr("fieldAlias"));
                });
                var s = selected.join();
                setSelectedColumnFields(s);
                $('.btn-Search').trigger('click');
            }
        }
    });
